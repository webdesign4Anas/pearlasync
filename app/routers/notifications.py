...
async def get_notifications(db:AsyncSession=Depends(get_db),owner:models.Users=Depends(oauth2.require_paid_business_owner)):
    result:Result=await db.execute(select(models.Notification).where(models.Notification.business_owner_id==owner.id).order_by(models.Notification.created_at.desc()))
